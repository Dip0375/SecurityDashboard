...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (!accountId || !name || !region || !accessKeyId || !secretAccessKey) {
      return res.status(400).json({
        error: "accountId, name, region, accessKeyId and secretAccessKey are required.",
      });
    }
    try {
      ...
      const { error: accountError } = await supabase.from(TABLE).upsert(
        { account_id: accountId, name, region, has_credentials: true, updated_at: now, created_at: now },
        { onConflict: "account_id" }
      );
      if (accountError) throw accountError;

      const { error: credentialsError } = await supabase.from(CREDENTIALS_TABLE).upsert(
        { account_id: accountId, encrypted_secret: encryptedSecret, updated_at: now, created_at: now },
        { onConflict: "account_id" }
      );
      if (credentialsError) throw credentialsError;

      return res.status(200).json({ id: accountId, name, region, hasCredentials: true, createdAt: now, updatedAt: now });
    } catch (err) {
  ...
  if (req.method === "DELETE") {
    ...
    try {
      ...
      await supabase.from(CREDENTIALS_TABLE).delete().eq("account_id", id);
