            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)" }}>
              {RELATIVE_GROUPS.map(group => (
                <div key={group.label} style={{ borderRight: `1px solid ${C.border}`, padding: "12px 0" }}>
                  <div style={{
                    color: C.textMut, fontSize: 10, fontWeight: 700, textTransform: "uppercase",
                    letterSpacing: "0.1em", padding: "0 12px 8px"
                  }}>{group.label}</div>
                  {group.options.map(opt => (
                    <button key={opt.value} onClick={() => { onChange({ type: "relative", value: opt.value }); setOpen(false); }}
                      style={{
                        ...btnBase, color: current.value === opt.value ? C.cyan : C.textSec,
          ...
          {tab === "absolute" && (
            <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <button onClick={applyAbsolute} disabled={!absStart || !absEnd || new Date(absEnd) <= new Date(absStart)}
                style={{
                  background: C.cyan, border: "none", borderRadius: 8, padding: "9px", color: C.bg,
...
// ─── Initial Credentials (provisioned via Vercel Environment Variables) ───────
function getEnvironmentCredentials() {
  const raw = import.meta.env.VITE_DASHBOARD_CREDENTIALS || import.meta.env.VITE_DEFAULT_CREDENTIALS;
  if (!raw) return [];

  try {
    const trimmed = raw.trim();
    // Handle cases where Vercel might wrap the value in extra quotes
    const normalized =
    ...
    console.warn("[Dashboard] Could not parse VITE_DASHBOARD_CREDENTIALS:", err.message);
    return [];
  }
}
...
function scaleAccount(acc, factor) {
  ...
  return {
    ...acc,
    waf: {
      ...acc.waf, allow: s(acc.waf.allow), block: s(acc.waf.block), count: s(acc.waf.count),
    ...
    guardDuty: {
      ...acc.guardDuty, findings: s(acc.guardDuty.findings), high: s(acc.guardDuty.high),
      medium: s(acc.guardDuty.medium), low: s(acc.guardDuty.low),
      types: acc.guardDuty.types.map(x => ({ ...x, count: s(x.count) }))
    },
    inspector: {
      ...acc.inspector, critical: s(acc.inspector.critical), high: s(acc.inspector.high),
      medium: s(acc.inspector.medium), low: s(acc.inspector.low)
    },
    securityHub: {
      ...acc.securityHub, critical: s(acc.securityHub.critical), high: s(acc.securityHub.high),
...
function Stat({ label, value, color = C.cyan, icon: Icon, sub }) {
  return (
    <div style={{
      ...
      <div style={{
        position: "absolute", top: 0, right: 0, width: 80, height: 80, borderRadius: "0 14px 0 80px",
        background: `${color}08`, pointerEvents: "none"
      }} />
      <div style={{
        display: "flex", alignItems: "center", gap: 7, color: C.textSec, fontSize: 11,
      ...
      <div style={{ ...card(), width: 460, boxShadow: "0 32px 80px rgba(0,0,0,0.8)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{
              width: 38, height: 38, borderRadius: 10, background: `${C.cyan}15`,
        ...
        {err && (
          <div style={{
            background: `${C.red}12`, border: `1px solid ${C.red}40`, borderRadius: 9,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
            <button onClick={handleSubmit}
              ...
              style={{
                flex: 1, background: `linear-gradient(135deg,${C.cyan},#006bff)`,
    ...
    <div style={{
      ...
      <div style={{ ...card(), width: 500, boxShadow: "0 32px 80px rgba(0,0,0,0.8)", maxHeight: "85vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{
              width: 38, height: 38, borderRadius: 10, background: `${C.yellow}15`,
        ...
        {/* Master toggle */}
        <div style={{
          ...card2(), display: "flex", justifyContent: "space-between", alignItems: "center",
          ...
          <button onClick={() => setEnabled(!enabled)} style={switchStyle(enabled)}>
            <div style={{
              position: "absolute", top: 2, left: enabled ? 20 : 2, width: 16, height: 16,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
          {eventList.map(ev => (
            <div key={ev.key} style={{
              ...card2(), display: "flex", justifyContent: "space-between",
              ...
              <button onClick={() => enabled && toggle(ev.key)} style={switchStyle(events[ev.key] && enabled)}>
                <div style={{
                  position: "absolute", top: 2, left: events[ev.key] && enabled ? 20 : 2, width: 16, height: 16,
                  borderRadius: 8, background: "white", transition: "left 0.2s"
                }} />
              </button>
            </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={save} style={{
            flex: 1, background: `linear-gradient(135deg,${C.yellow},${C.orange})`,
...
function ExecutiveReportModal({ accounts, onClose }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: "#fff", width: "100%", maxWidth: 900, height: "90vh", borderRadius: 16,
        ...
        <div className="no-print" style={{
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button onClick={onClose} style={{
              ...
              padding: "8px 16px", color: "#64748b", fontWeight: 600, cursor: "pointer"
            }}>Close</button>
          </div>
        </div>
...
function LoginScreen({ onLogin, credentials }) {
  ...
  const inp = {
    background: `${C.card2}cc`, border: `1px solid ${C.border2}`, borderRadius: 10,
  ...
  return (
    <div style={{
      ...
      <div style={{ ...card(), width: 420, position: "relative", boxShadow: `0 32px 80px rgba(0,0,0,0.8), 0 0 0 1px ${C.border2}` }}>
        ...
        {err && (
          <div style={{
            background: "rgba(255,59,92,0.1)", border: `1px solid ${C.red}40`,
...
function Sidebar({ active, setActive, role, user, onLogout, onChangePassword, onNotifSettings }) {
  ...
  return (
    <div style={{
      width: w, minHeight: "100vh", background: C.surface, borderRight: `1px solid ${C.border}`,
      display: "flex", flexDirection: "column", transition: "width 0.22s ease", flexShrink: 0, zIndex: 50
    }}>
      <div style={{
        padding: collapsed ? "14px 0" : "18px 16px 14px", borderBottom: `1px solid ${C.border}`,
        display: "flex", alignItems: "center", justifyContent: collapsed ? "center" : "space-between", minHeight: 60
      }}>
        {!collapsed && (
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{
              width: 32, height: 32, borderRadius: 9, background: `${C.cyan}18`,
        ...
        <button onClick={() => setCollapsed(!collapsed)} style={{
          background: `${C.border}80`, border: "none",
      ...
      <div style={{ borderTop: `1px solid ${C.border}`, padding: collapsed ? "10px 0" : "12px 14px" }}>
        {!collapsed && (
          <div style={{
            ...
            <div style={{
              width: 30, height: 30, borderRadius: "50%", background: `${C.purple}25`,
...
function Header({ accounts, selected, setSelected, section, timeRange, setTimeRange, currentUser, onChangePassword, onNotifSettings, onRefresh, refreshing }) {
  ...
  return (
    <div style={{
      background: C.surface, borderBottom: `1px solid ${C.border}`, padding: "12px 24px",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        ...
        <button onClick={onRefresh} disabled={refreshing}
          style={{
            width: 36, height: 36, borderRadius: 8, background: C.card2, border: `1px solid ${C.border2}`,
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18, maxWidth: 600 }}>
      ...
      <div style={card()}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{
            width: 52, height: 52, borderRadius: "50%", background: `${C.purple}25`,
      ...
      <div style={card()}>
        ...
        <div style={{
          ...
          <button onClick={() => setShowChangePwd(true)}
            style={{
              background: `${C.cyan}15`, border: `1px solid ${C.cyan}30`, borderRadius: 8,
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {[
            ...
            <div key={item.label} style={{
              background: item.active ? `${C.green}12` : `${C.border}50`,
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input id="resend-key-input" type="password" placeholder="re_••••••••••••••••••••"
                  style={{
                    flex: 1, background: C.card2, border: `1px solid ${C.border2}`, borderRadius: 8,
  ...
  if (accounts.length === 0) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 400, gap: 20 }}>
        <div style={{
          width: 80, height: 80, borderRadius: 16, background: `${C.cyan}15`,
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <button onClick={() => setActive("accounts")}
            style={{
              background: `linear-gradient(135deg,${C.cyan},${C.cyanDim})`, border: "none",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        ...
        <button onClick={() => setShowReport(true)}
          style={{
            display: "flex", alignItems: "center", gap: 8, background: `${C.cyan}18`, border: `1px solid ${C.cyan}40`,
      ...
      <div style={card()}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {gd.types.length === 0 ? (
            ...
            <div key={i} style={{
              ...card2(), display: "flex", alignItems: "center", gap: 14,
              borderLeft: `3px solid ${sevColor(t.severity)}`
            }}>
              <div style={{
                width: 40, height: 40, borderRadius: 10, background: sevBg(t.severity),
                display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0
              }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{
                  color: C.textPri, fontSize: 13, fontWeight: 600, fontFamily: "monospace",
          ...
          <thead>
            <tr style={{ borderBottom: `1px solid ${C.border}` }}>
              {["Resource", "CVE / Finding", "Severity"].map(h => (
                <th key={h} style={{
                  color: C.textSec, padding: "8px 14px", textAlign: "left",
      ...
      {showAdd && (
        <div style={card()}>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <button onClick={addAccount} disabled={saving} style={{
              background: saving ? C.accent : C.cyan, border: "none", borderRadius: 9,
              ...
              <tbody>
                {rows.map((row, i) => (
                  <tr key={i} style={{ borderBottom: `1px solid ${C.border}20` }}
                    ...
                    {columns.map(c => (
                      <td key={c.key} style={{
                        padding: "8px 10px", color: c.color ? c.color(row[c.key]) : C.textPri,
  ...
  if (error) return (
    <div style={{ ...card(), display: "flex", flexDirection: "column", gap: 12, alignItems: "center", padding: 32, textAlign: "center" }}>
      ...
      <button onClick={load} style={{
        background: C.cyan, border: "none", borderRadius: 8, padding: "8px 20px",
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        ...
        {["all", "success", "warning", "info"].map(f => (
          <button key={f} onClick={() => { setFilter(f); setPage(1); }}
            style={{
              background: filter === f ? `${C.cyan}15` : C.card2,
            ...
            <thead>
              <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                {["Timestamp", "User", "Action", "IP", "Detail", "Status"].map(h => (
                  <th key={h} style={{
                    color: C.textSec, padding: "8px 12px", textAlign: "left",
              ...
              {page_items.map(e => {
                ...
                return (
                  <tr key={e.id} style={{ borderBottom: `1px solid ${C.border}22`, transition: "background 0.1s" }}
                    ...
                    <td style={{
                      padding: "9px 12px", color: C.textSec, fontSize: 11, maxWidth: 280,
          ...
          {totalPages > 1 && (
            <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 6, marginTop: 16 }}>
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                style={{
                  background: C.card2, border: `1px solid ${C.border2}`, borderRadius: 7,
              ...
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                ...
                return (
                  <button key={pg} onClick={() => setPage(pg)}
                    style={{
                      background: pg === page ? C.cyan : C.card2, border: `1px solid ${pg === page ? C.cyan : C.border2}`,
